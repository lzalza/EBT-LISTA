...
class Problem(object):
    ...
    def gen_samples(self, size, pnz=None, SNR=None, probability=None):
        ...
        print(probability)
        if(probability == 0):
            prob = (np.random.uniform(size=(self.N,size)) <= 0.5).astype (np.float32) * 0.05 + 0.05
            bernoulli = np.random.uniform(size=(self.N,size)) <= prob
            bernoulli = bernoulli.astype (np.float32)
            x = bernoulli * np.random.normal (size=(self.N, size)).astype(np.float32)
