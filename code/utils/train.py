...
def setup_input_sc(test, p, tbs, vbs, fixval, supp_prob, SNR,
    ...
    with tf.name_scope('input'):
        ...
        #mix pnz
            temp_prob =  tf.constant(value=0.5, dtype=tf.float32, name='temp_prob')
            prob_ = tf.random_uniform(shape=(1, tbs), minval=0.0, maxval=1.0,
                                      name='prob')
            prob_ = tf.to_float(prob_ <= temp_prob) * 0.05 +0.05
            ...
            if not test:
                prob_val_ = tf.random_uniform(shape=(1, vbs), minval=0.0, maxval=1.0,
                                      name='prob_val')
                prob_val_ = tf.to_float(prob_val_ <= temp_prob) * 0.1 +0.1
                supp_val_ = tf.random_uniform(shape=(N, vbs),
                                              minval=0.0, maxval=1.0,
                                              dtype=tf.float32, name='supp_val')
                supp_val_ = tf.to_float(supp_val_ <= prob_val_)
        ...
        if magdist == 'normal':
            ...
            if not test:
                mag_val_ = tf.random_normal (shape=(N, vbs),
                                             ...
                                             dtype=tf.float32,
                                             name='mag')
        elif magdist == 'mix_normal':
           mag1_ = tf.random_normal(shape=(N, tbs),
                                    mean=0.0,
                                    stddev=1.0,
                                    dtype=tf.float32,
                                    name='mag1')
           mag2_ = tf.random_normal(shape=(N, tbs),
                                    mean=distargs ['mean'],
                                    stddev=distargs['std'],
                                    dtype=tf.float32,
                                    name='mag2')
           sel_ = (tf.random_uniform(shape=(N, tbs), minval=0.0, maxval=1.0,
                                      dtype=tf.float32)
                    <= 0.5)
           sel1_ = tf.to_float(sel_)
           sel2_ = tf.to_float(tf.logical_not(sel_))
           mag_ = mag1_ * sel1_ + mag2_ * sel2_
           if not test:
               mag1_val_ = tf.random_normal(shape=(N, vbs),
                                            mean=0.0,
                                            stddev=1.0,
                                            dtype=tf.float32,
                                            name='mag1_val')
               mag2_val_ = tf.random_normal(shape=(N, vbs),
                                            mean=distargs ['mean'],
                                            stddev=distargs['std'],
                                            dtype=tf.float32,
                                            name='mag2_val')
               sel_val_ = (tf.random_uniform(shape=(N, vbs), minval=0.0, maxval=1.0,
                                         dtype=tf.float32)
                           <= 0.5)       
               sel1_val_ = tf.to_float(sel_val_)
               sel2_val_ = tf.to_float(tf.logical_not(sel_val_))
               mag_val_ = mag1_val_ * sel1_val_ + mag2_val_ * sel2_val_ 
...
def load_trainable_variables (sess, filename):
    ...
    for k, d in np.load (filename).items ():
        if k in tv:
            print ('restoring ' + k)
            print(sess.run (tf.assign (tv[k], d)))
        else:
            other [k] = d
