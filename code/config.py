...
net_arg.add_argument (
    '-a', '--alti', type=float, default=5.0,
    help="")
net_arg.add_argument (
    '-o' , '--overshoot' , action='store_true',
    help="Whether overshoot gate is adopted.")
net_arg.add_argument (
    '-gain' , '--gain' , action='store_true',
    help="Whether gain gate is adopted.")
net_arg.add_argument(
        '-uf','--use_function',type = str,default="relu",help="Used function in the gain gate and overshoot gate")
net_arg.add_argument(
        '-gf','--gain_fun',type = str,default="none",help="Used function in the gain gate and overshoot gate")
net_arg.add_argument(
        '-of','--over_fun',type = str,default="none",help="Used function in the gain gate and overshoot gate")
net_arg.add_argument (
    '-bg' , '--both_gate' , action='store_true',
    help="both_gate")
net_arg.add_argument(
'-T_c', '--T_combine', type=int, default=10,
    help="Number of Block of LISTA."
)
net_arg.add_argument(
'-T_m', '--T_middle', type=int, default=6,
    help="Number of Block of LISTA."
)
net_arg.add_argument(
'-ad','--adaptive',action='store_true', help = 'Whether use adaptive threshold'
)
net_arg.add_argument(
'-bias', '--bias', action='store_true', help = 'Whether use bias adaptive threshold'
)
net_arg.add_argument(
'-norm','--normalize',action='store_true',help = 'Whether normalize input Y'
)
net_arg.add_argument(
'-fun','--function',type=int, default=0,help = 'function type'
)
    ...
    if config.task_type == 'sc':
        ...
        if config.magdist == 'normal':
            ...
            config.distargs = dict(p=config.magbp,
                                   v0=config.magbv0,
                                   v1=config.magbv1)
        elif config.magdist == 'mix_normal':
            config.distargs = dict(mean=config.magnmean,
                                   std=config.magnstd)
